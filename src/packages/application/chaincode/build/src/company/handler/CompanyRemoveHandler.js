...
let CompanyRemoveHandler = class CompanyRemoveHandler extends handler_1.TransportCommandFabricAsyncHandler {
    constructor(logger, transport) {
        super(logger, transport, company_2.CompanyRemoveCommand.NAME);
    }
    async execute(params, holder) {
        ...
        if (!item.wallet.isEmpty()) {
            throw new error_1.LedgerError(error_1.LedgerErrorCode.NOT_FOUND, `Unable to remove company ${params.uid}: wallet is not empty`);
        }
        throw new error_1.LedgerError(error_1.LedgerErrorCode.BAD_REQUEST, `Unable to remove company: it's deprecated now`);
        await holder.db.company.remove(params.uid);
        await holder.stub.dispatch(new company_1.CompanyRemovedEvent(holder.eventData));
    }
};
__decorate([
    (0, guard_1.UserGuard)(),
...
], CompanyRemoveHandler.prototype, "execute", null);
CompanyRemoveHandler = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [logger_1.Logger, chaincode_1.TransportFabricChaincodeReceiver])
], CompanyRemoveHandler);
exports.CompanyRemoveHandler = CompanyRemoveHandler;
