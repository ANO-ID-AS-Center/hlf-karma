...
exports.ProjectRemoveHandler = void 0;
...
let ProjectRemoveHandler = class ProjectRemoveHandler extends handler_1.TransportCommandFabricAsyncHandler {
    constructor(logger, transport) {
        super(logger, transport, project_1.ProjectRemoveCommand.NAME);
    }
    async execute(params, holder) {
        ...
        if (!item.wallet.isEmpty()) {
            throw new error_1.LedgerError(error_1.LedgerErrorCode.NOT_FOUND, `Unable to remove project ${params.uid}: wallet is not empty`);
        }
        throw new error_1.LedgerError(error_1.LedgerErrorCode.BAD_REQUEST, `Unable to remove project: it's deprecated now`);
        await holder.db.project.remove(item);
        await holder.stub.dispatch(new project_2.ProjectRemovedEvent(holder.eventData));
    }
};
__decorate([
    (0, guard_1.UserGuard)(),
...
], ProjectRemoveHandler.prototype, "execute", null);
ProjectRemoveHandler = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [logger_1.Logger, chaincode_1.TransportFabricChaincodeReceiver])
], ProjectRemoveHandler);
exports.ProjectRemoveHandler = ProjectRemoveHandler;
